...
function apply_data_row_style($sheet, int $rowNumber): void {
  ...
  $sheet->getStyle("E{$rowNumber}:S{$rowNumber}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
}
...
try {
  ...
  if ($barangayId <= 0) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'monitoring_0_23_months', null, 'Invalid barangay_id');
    json_out(422, ["ok" => false, "message" => "Invalid barangay"]);
  }

  if ($year < 2000 || $year > 2100) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'monitoring_0_23_months', (string)$barangayId, 'Invalid year');
    json_out(422, ["ok" => false, "message" => "Invalid year"]);
  }

  if ($month < 1 || $month > 12) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'monitoring_0_23_months', (string)$barangayId, 'Invalid month');
  ...
  if ($role !== 'admin') {
    ...
    if ($userBarangayId <= 0) {
      audit_log($pdo, $userId, 'REPORT_EXPORT_DENIED', 'monitoring_0_23_months', (string)$barangayId, 'No barangay assigned');
      json_out(403, ["ok" => false, "message" => "No barangay assigned"]);
    }

    if ($userBarangayId !== $barangayId) {
      audit_log($pdo, $userId, 'REPORT_EXPORT_DENIED', 'monitoring_0_23_months', (string)$barangayId, 'User attempted access to another barangay report');
  ...
  if (!$barangay) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'monitoring_0_23_months', (string)$barangayId, 'Barangay not found');
  ...
  foreach ($rawRows as $row) {
    $rows[] = [
      ...
      'height_value' => $row['height_value'] !== null && $row['height_value'] !== '' ? (float)$row['height_value'] : '',
      'weight_value' => $row['weight_value'] !== null && $row['weight_value'] !== '' ? (float)$row['weight_value'] : '',
      'muac_value' => $row['muac_value'] !== null && $row['muac_value'] !== '' ? (float)$row['muac_value'] : '',
      'weight_status' => trim((string)($row['weight_status'] ?? '')),
      'height_status' => trim((string)($row['height_status'] ?? '')),
      'lt_status' => trim((string)($row['lt_status'] ?? '')),
      'muac_status' => trim((string)($row['muac_status'] ?? ''))
    ];
  }
  ...
  $templatePath = __DIR__ . "/templates/monitoring_0_23_months.xlsx";
  if (!file_exists($templatePath)) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'monitoring_0_23_months', (string)$barangayId, 'Excel template not found');
  ...
  $sheet->setCellValue('J8', $totalChildren);
  $sheet->setCellValue('X8', max(1, (int)ceil(max($totalChildren, 1) / 25)));
  ...
  if ($totalRows === 0) {
    ...
    $sheet->setCellValue("A{$startRow}", 'No child records found for the selected barangay, month, and year.');
    $sheet->getStyle("A{$startRow}:S{$startRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle("A{$startRow}:S{$startRow}")->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    $sheet->getStyle("A{$startRow}:S{$startRow}")->getAlignment()->setWrapText(true);
    ...
    foreach ($rows as $row) {
      ...
      if (!empty($row['birthdate'])) {
        ...
        $sheet->getStyle("F{$rowNumber}")->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_DATE_YYYYMMDD2);
      } else {
      ...
      $sheet->setCellValue("J{$rowNumber}", $row['weight_status']);
      $sheet->setCellValue("K{$rowNumber}", $row['height_status']);
      $sheet->setCellValue("L{$rowNumber}", $row['lt_status']);
      $sheet->setCellValue("M{$rowNumber}", $row['muac_status']);

      for ($col = 14; $col <= 19; $col++) {
        $sheet->setCellValue(Coordinate::stringFromColumnIndex($col) . $rowNumber, '');
      }
    ...
    $sheet->getStyle("G{$startRow}:I" . ($startRow + $totalRows - 1))
      ->getNumberFormat()
      ->setFormatCode('0.00');
  }

  $filename = sprintf(
    'Monitoring_List_0_23_Months_%s_%04d_%02d.xlsx',
  ...
  audit_log(
    ...
    $userId,
    'REPORT_EXPORTED',
    'monitoring_0_23_months',
    (string)$barangayId,
    "Exported Monitoring List 0-23 Months for barangay={$barangay['barangay_name']} ({$barangayId}), period={$year}-" .
