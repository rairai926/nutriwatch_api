...
try {
  ...
  foreach ($rawRows as $row) {
    $muacStatus = normalize_muac($row['muac_status'] ?? '');
  ...
  $templatePath = __DIR__ . "/templates/list_muac.xlsx";
  ...
  if ($totalChildren === 0) {
    ...
    $sheet->setCellValue("A{$startRow}", 'No children with MUAC wasting status found for the selected barangay, month, and year.');
    ...
    foreach ($rows as $row) {
      ...
      $sheet->setCellValue("J{$rowNumber}", $row['muac_status']);
  ...
  $filename = sprintf(
    'MUAC_Children_0_59_Months_%s_%04d_%02d.xlsx',
  ...
  audit_log(
    ...
    'muac_0_59_months',
    (string)$barangayId,
    "Exported MUAC 0-59 Months, rows={$totalChildren}, measurements_marked={$exportedCount}"
  );
