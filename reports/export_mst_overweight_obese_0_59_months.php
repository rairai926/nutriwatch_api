...
try {
  ...
  foreach ($rawRows as $row) {
    ...
    if ($hfa !== 'MSt' && !in_array($wfl, ['OW', 'Ob'], true)) {
      continue;
    }
  ...
  $templatePath = __DIR__ . "/templates/list_mst_ov_ob.xlsx";
  ...
  if ($totalChildren === 0) {
    ...
    $sheet->setCellValue("A{$startRow}", 'No moderately stunted, overweight, or obese children found for the selected barangay, month, and year.');
    ...
    foreach ($rows as $row) {
      ...
      $sheet->setCellValue("J{$rowNumber}", $row['wfl']);
  ...
  $filename = sprintf(
    'MSt_Overweight_Obese_Children_0_59_Months_%s_%04d_%02d.xlsx',
  ...
  audit_log(
    ...
    $userId,
    'REPORT_EXPORTED',
    'mst_overweight_obese_0_59_months',
    (string)$barangayId,
    "Exported MSt/Overweight/Obese 0-59 Months, rows={$totalChildren}, measurements_marked={$exportedCount}"
  );
