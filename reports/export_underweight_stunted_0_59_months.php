...
function apply_data_row_style($sheet, int $rowNumber): void {
  ...
  $sheet->getStyle("E{$rowNumber}:N{$rowNumber}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
}
...
try {
  ...
  foreach ($rawRows as $row) {
    ...
    if (!in_array($wfa, ['MUW', 'SUW'], true) && !in_array($hfa, ['MSt', 'SSt'], true)) {
      continue;
    }
  ...
  $templatePath = __DIR__ . "/templates/list_sumsst.xlsx";
  ...
  $sheet->setTitle('Underweight Stunted');
  ...
  $sheet->setCellValue('N8', $pageCount);
  ...
  if ($totalChildren === 0) {
    ...
    $sheet->setCellValue("A{$startRow}", 'No moderately/severely underweight or stunted children found for the selected barangay, month, and year.');
    $sheet->getStyle("A{$startRow}:N{$startRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    $sheet->getStyle("A{$startRow}:N{$startRow}")->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    ...
    foreach ($rows as $row) {
      ...
      $sheet->setCellValue("G{$rowNumber}", $row['weight_value']);
      $sheet->setCellValue("H{$rowNumber}", $row['wfa']);
      $sheet->setCellValue("I{$rowNumber}", $row['hfa']);
    ...
    $sheet->getStyle("G{$startRow}:G" . ($startRow + $totalChildren - 1))
      ->getNumberFormat()
      ->setFormatCode('0.00');
  }

  $filename = sprintf(
    'Underweight_Stunted_Children_0_59_Months_%s_%04d_%02d.xlsx',
  ...
  audit_log(
    ...
    $userId,
    'REPORT_EXPORTED',
    'underweight_stunted_0_59_months',
    (string)$barangayId,
    "Exported Underweight/Stunted 0-59 Months, rows={$totalChildren}, measurements_marked={$exportedCount}"
  );
