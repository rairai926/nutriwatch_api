...
try {
  ...
  if ($barangayId <= 0) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'moderately_wasted_0_59_months', null, 'Invalid barangay_id');
    json_out(422, ["ok" => false, "message" => "Invalid barangay"]);
  }

  if ($year < 2000 || $year > 2100) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'moderately_wasted_0_59_months', (string)$barangayId, 'Invalid year');
    json_out(422, ["ok" => false, "message" => "Invalid year"]);
  }

  if ($month < 1 || $month > 12) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'moderately_wasted_0_59_months', (string)$barangayId, 'Invalid month');
  ...
  if ($role !== 'admin') {
    ...
    if ($userBarangayId <= 0) {
      audit_log($pdo, $userId, 'REPORT_EXPORT_DENIED', 'moderately_wasted_0_59_months', (string)$barangayId, 'No barangay assigned');
      json_out(403, ["ok" => false, "message" => "No barangay assigned"]);
    }

    if ($userBarangayId !== $barangayId) {
      audit_log($pdo, $userId, 'REPORT_EXPORT_DENIED', 'moderately_wasted_0_59_months', (string)$barangayId, 'User attempted access to another barangay report');
  ...
  if (!$barangay) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'moderately_wasted_0_59_months', (string)$barangayId, 'Barangay not found');
  ...
  foreach ($rawRows as $row) {
    ...
    if ($wfl !== 'MW/MAM' && $muacStatus !== 'MW/MAM') {
      continue;
    }
  ...
  $templatePath = __DIR__ . "/templates/list_moderately_wasted_mam_0_59_months.xlsx";
  if (!file_exists($templatePath)) {
    audit_log($pdo, $userId, 'REPORT_EXPORT_FAILED', 'moderately_wasted_0_59_months', (string)$barangayId, 'Excel template not found');
    json_out(500, ["ok" => false, "message" => "Excel template not found"]);
  }

  $spreadsheet = IOFactory::load($templatePath);
  $sheet = $spreadsheet->getActiveSheet();
  $sheet->setTitle('Moderately Wasted');

  $baseTemplateRows = 14;
  $startRow = 11;
  $pageCount = max(1, (int)ceil(max($totalChildren, 1) / $baseTemplateRows));

  // clear external-link formulas from the template
  ...
  if ($totalChildren === 0) {
    ...
    $sheet->setCellValue("A{$startRow}", 'No moderately wasted (MAM) children found for the selected barangay, month, and year.');
    ...
    foreach ($rows as $row) {
      ...
      // Follow-up visit columns J:O left blank for manual use
  ...
  $filename = sprintf(
    'Moderately_Wasted_MAM_Children_0_59_Months_%s_%04d_%02d.xlsx',
  ...
  audit_log(
    ...
    $userId,
    'REPORT_EXPORTED',
    'moderately_wasted_0_59_months',
    (string)$barangayId,
    "Exported Moderately Wasted (MAM) 0-59 Months for barangay={$barangay['barangay_name']} ({$barangayId}), period={$year}-" .
